...
def cal_eval_metric(best_pred, preds, answers):
    ...
    if len(answers) == 0:
        ...
        if total == 0:
            ...
            f1 = 2.0 / (1.0 / precision + 1.0 /
                        recall) if precision != 0 and recall != 0 else 0.0
            return precision, recall, f1, hits


def compare_pr(kb_pred_file, doc_pred_file, hybrid_pred_file, w_kb, eps_doc,
               eps_kb, eps_ensemble, eps_hybrid, eps_ensemble_all):
    ...
    with open(kb_pred_file) as f_kb, open(doc_pred_file) as f_doc, open(
            hybrid_pred_file) as f_hybrid:
        line_id = 0
        for line_kb, line_doc, line_hybrid in tqdm(zip(f_kb, f_doc, f_hybrid)):
            ...
            assert line_kb['answers'] == line_doc['answers'] == line_hybrid[
                'answers']
            ...
            dist_ensemble_all = combine_dist(dist_ensemble,
                                             dist_hybrid,
                                             w1=0.3)
            ...
            p, r, f1, hits = get_one_f1(doc_entities, dist_doc, eps_doc,
                                        answers)
            ...
            p, r, f1, hits = get_one_f1(either_entities, dist_ensemble,
                                        eps_ensemble, answers)
            ensemble_precision.append(p)
            ensemble_recall.append(r)
            ensemble_f1.append(f1)
            ensemble_hits.append(hits)

            p, r, f1, hits = get_one_f1(either_entities, dist_hybrid,
                                        eps_hybrid, answers)
            ...
            p, r, f1, hits = get_one_f1(either_entities, dist_ensemble_all,
                                        eps_ensemble_all, answers)
    ...
    print('precision: ',
          sum(ensemble_all_precision) / len(ensemble_all_precision))
...
if __name__ == "__main__":
    ...
    compare_pr(pred_kb_file, pred_doc_file, pred_hybrid_file, w_kb, eps_doc,
               eps_kb, eps_ensemble, eps_hybrid, eps_ensemble_all)
