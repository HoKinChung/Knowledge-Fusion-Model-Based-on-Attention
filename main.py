    ...
    for epoch in range(cfg['num_epoch']):
        try:
            ...
            for iteration in tqdm(
                ...
                optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(my_model.parameters(),
                                               cfg['gradient_clip'])
                optimizer.step()
            print('avg_training_loss', sum(train_loss) / len(train_loss))
            tf_logger.add_scalar('train_loss',
                                 sum(train_loss) / len(train_loss), epoch)
            tf_logger.add_scalar('avg_train_acc',
                                 sum(train_acc) / len(train_acc), epoch)
...
def inference(my_model, valid_data, entity2id, cfg, log_info=False):
    ...
    for iteration in tqdm(range(valid_data.num_data // test_batch_size + 1)):
        batch = valid_data.get_batch(iteration,
                                     test_batch_size,
                                     fact_dropout=0.0)
        ...
        acc, max_acc = cal_accuracy(pred, batch[-2])
        if log_info:
            output_pred_dist(pred_dist, batch[-2], id2entity,
                             iteration * test_batch_size, valid_data, f_pred)
